...
public class RotateScript : MonoBehaviour {
	...
	public void SetDrawFeedback(bool hasDrawFeedback){
		...
		if (hasDrawFeedback){
			...
			Vector3 grootte;
			float schaal;
			if (selectedObject.GetComponent<Renderer>().bounds.size.x > selectedObject.GetComponent<Renderer>().bounds.size.z){
				schaal = selectedObject.GetComponent<Renderer>().bounds.size.x / clone.GetComponent<Renderer>().bounds.size.x;
			} else {
				schaal = selectedObject.GetComponent<Renderer>().bounds.size.z / clone.GetComponent<Renderer>().bounds.size.z;
			}
			grootte.x = schaal;
			grootte.z = schaal;	
			grootte.y = 0.05f;
